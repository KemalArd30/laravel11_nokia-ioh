...
use App\Models\WCCFullPaymentData;
use Illuminate\Support\Facades\Log;
use Maatwebsite\Excel\Concerns\ToModel;
use Maatwebsite\Excel\Concerns\WithHeadingRow;
use PhpOffice\PhpSpreadsheet\Shared\Date;

class WCCFullPaymentDataImport implements ToModel, WithHeadingRow
{
    public function __construct()
    {
        // Hapus semua data dari tabel sebelum impor
        WCCFullPaymentData::truncate();
        Log::info('All previous data deleted from wcc_full_payment_data table.');
    }
    ...
    public function model(array $row)
    {
        ...
        $data = new WCCFullPaymentData([
            ...
            'gr_status' => isset($row['gr_status']) ? $this->convertDate($row['gr_status']) : null,
            ...
            'aging_wcc_approval' => $row['aging_wcc_approval'] ?? null,
            'wcc_approval_categorize' => $wccApprovalCategorize,
            'ms_start_date' => isset($row['ms_start_date']) ? $this->convertDate($row['ms_start_date']) : null,
